...
class Blockchain(object):
    ...
    def new_block(self, proof, previous_hash=None):
        ...
        * The hash of the previous block
        ...
        self.current_transactions = []
        # Append the chain to the block  ?? append block to the chain instead?
    ...
    @property   #built-in Python decorator, make it available to be used as a property
    def last_block(self):
        return self.chain[-1]
...
def mine():
    ...
    if not data['proof'] or not data['id']:
        response = {
            'message': 'proof and id are required'
        }
        ...
        if blockchain.valid_proof(block_string, data['proof']):
            ...
            blockchain.new_transaction('0', data['id'], 1) #add new transaction
            ...
            return jsonify({
                'message': 'Incorrect proof or block already mined'
            }), 404
...
def get_new_transaction():
    data = request.get_json()
    requests = ['sender', 'recipient', 'amount']
    if not all(k in data for k in requests):
        response = {
        ...
        response = {
            'message': f'transaction will be added to block index {index}'
        }
        return jsonify(response), 200
